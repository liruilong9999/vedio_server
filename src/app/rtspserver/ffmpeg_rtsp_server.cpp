...
ServerMediaSession *FfmpegRTSPServer::lookupServerMediaSession(const char *streamName, Boolean isFirstLookupInSession)
{
	...
	printf("before DynamicRTSPServer::lookupServerMediaSession\n");
...
static ServerMediaSession* createNewSMS(UsageEnvironment& env,
    ...
    if (strcmp(extension, ".avi") == 0 
        ...
        if (demux != NULL) {
			ServerMediaSubsession *temp = demux->NewVideoServerMediaSubsession();
			if (temp != NULL)
				sms->addSubsession(temp);

			temp = demux->NewAudioServerMediaSubsession();
			if (temp != NULL)
				sms->addSubsession(temp);
			//sms->addSubsession(WAVAudioFileServerMediaSubsession::createNew(env, fileName, reuseSource, true));
			Sleep(100);
